...
DesPath = './datasets2/HelenFace/'
# FileName = 'ShortTransformMeanShapeSet'+str(dataset_size) +'.json'
FileName = 'ShortShapeSet_Helen'+ str(dataset_size) +'.json'              #在当前实验中,并未使用转换成固定规格之后的图片
...
model1 = classifier.CLASSIFIER(train_feature_list, label_list, input_nc, pool_nd, len(SuitableObjeID), 0.001, 0.5, 2, 1) #14
model1.fit()
...
if opt.gpu_ids[0] == 1:
    pretrain_classifier_path2 = 'TE_pretrained_classifier_gpu1_Helen.pkl'   # Tr: 'ED_pretrained_classifier_gpu1_Helen.pkl' Te: 'TE_pretrained_classifier_gpu1_Helen.pkl'
elif opt.gpu_ids[0] == 0:
    pretrain_classifier_path2 = 'TE_pretrained_classifier_gpu0_Helen.pkl'   # Tr: 'ED_pretrained_classifier_gpu0_Helen.pkl' Te: 'TE_pretrained_classifier_gpu0_Helen.pkl'
