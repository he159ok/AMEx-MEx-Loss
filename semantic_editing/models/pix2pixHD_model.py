...
class Pix2PixHDModel(BaseModel):
    ...
    def encode_input(self, label_map, inst_map=None, real_image=None, feat_map=None, infer=False):             
        if self.opt.label_nc == 0:
            ...
            # print("now self.opt.label_nc is:", self.opt.label_nc)
            if self.opt.dataName == 'cityscape':
                oneHot_size = (size[0], 35, size[2], size[3])
            elif self.opt.dataName == 'NYU':
                oneHot_size = (size[0], 895, size[2], size[3])
            elif self.opt.dataName == 'helen':
	            oneHot_size = (size[0], 12, size[2], size[3])
            ...
            for i in range(self.opt.num_D):
                for j in range(len(pred_fake[i])-1):
                    loss_G_GAN_Feat += D_weights * feat_weights * \
                        self.criterionFeat(pred_fake[i][j], pred_real[i][j].detach()) * self.opt.lambda_feat          #从二者一起的角度来看，在做feature matching
