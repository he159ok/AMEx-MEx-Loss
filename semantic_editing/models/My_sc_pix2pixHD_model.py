...
class MyScPix2PixHDModel(BaseModel):
	def name(self):
	...
	def initialize(self, opt):
		...
		input_nc = (opt.label_nc+3) if opt.label_nc != 0 else opt.input_nc
	...
	def forward(self, InComLableMap, InComInstMap, InComBinAdj, InComRealAdj, ComRealAdj, classNum, instNumEachClass, InComImg, ComImage, feat, InitNodeEmb, infer=False):
		# Encode Inputs
		...
		NodeEmbedding = self.scNodeEmb(InitNodeEmb)                  # X in EGCN
		InComEdgeEmbedding = self.scEdgeEmb(InComRealAdj.long()).permute(0, 2, 1)             # A的一部分inGCN
		InComScGraphEmbedding, _ = self.scEGCN((NodeEmbedding, InComEdgeEmbedding))
		ComEdgeEmbedding = self.scEdgeEmb(ComRealAdj.long()).permute(0, 2, 1)             # A的一部分inGCN
		ComScGraphEmbedding, _ = self.scEGCN((NodeEmbedding, ComEdgeEmbedding))

		if self.is_scGraph:
			_, _, hei = ComScGraphEmbedding.shape
			InComScGraphEmbedding = InComScGraphEmbedding.permute(1, 0, 2)
			ComScGraphEmbedding = ComScGraphEmbedding.permute(1, 0, 2)
			PoolingMid = torch.nn.AdaptiveAvgPool2d([1, hei])
			InComScGraphEmbedding = PoolingMid(InComScGraphEmbedding).squeeze(1).unsqueeze(0).unsqueeze(0)
			ComScGraphEmbedding = PoolingMid(ComScGraphEmbedding).squeeze(1).unsqueeze(0).unsqueeze(0)

			DifScGraphEmbedding = ComScGraphEmbedding - InComScGraphEmbedding

			input_label = torch.cat((input_label, InComScGraphEmbedding), dim=1)
			input_label = torch.cat((input_label, ComScGraphEmbedding), dim=1)
			input_label = torch.cat((input_label, DifScGraphEmbedding), dim=1)
...
class MyScInferenceModel(MyScPix2PixHDModel):
