...
class GraphTripleConv(nn.Module):
  ...
  def forward(self, obj_vecs, pred_vecs, edges):
    ...
    cur_s_vecs = obj_vecs[s_idx]      #obj_vecs 此时是 物体类别数×embedding维度数;;; cur_s_vecs此时是  当前batch中的物体总数量×embedding维度数
    ...
    new_p_vecs = new_t_vecs[:, H:(H+Dout)]    # p是边，此时只有Dout维度数
    ...
    pooled_obj_vecs = pooled_obj_vecs.scatter_add(0, s_idx_exp, new_s_vecs)  #个人感觉此时并未加全全部的东西，所以第三个参数的张量其实有一些未用到。
