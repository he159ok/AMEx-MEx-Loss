...
class MyBasePix2PixHDModel(BaseModel):
	...
	def forward(self, InComLableMap, InComInstMap, SelectLocLayer, InComImg, ComImage, SelectedBox, pretrained_cls, SeleL2ProjL, SelectC, feat, ComLabelMap):
		...
		loss_G_VGG = 0  # 计算loss:perception loss，在这个版本中用于计算重建损失
		...
		if not self.opt.no_vgg_loss:
			...
			loss_recon_comb = self.criterionCtxRecon(fake_image, ComLabelMap.detach().long().squeeze(1), comb_gt_mask)

			obj_generate_part1 = self.find_local_object(fake_image, SelectC)
			obj_generate_part2 = self.mask_variable(obj_generate_part1.unsqueeze(0), SelectLocLayer.detach())

			obj_gt_part1 = self.find_local_object(com_label, SelectC)
			obj_gt_part2 = self.mask_variable(obj_gt_part1.unsqueeze(0), SelectLocLayer.detach())
		...
		output_image = [fake_whole_sym_label, fake_masked_sym_label]
	...
	def inference(self, InComLableMap, InComInstMap, SelectLocLayer, InComImg, ComImage, SelectedBox, feat):
		...
		fake_masked_syn_image[:, :, SelectedBox[0]:SelectedBox[1], SelectedBox[2]:SelectedBox[3]] = fake_image[:, :,
		                                                                                            SelectedBox[0]:
		                                                                                            SelectedBox[1],
		                                                                                            SelectedBox[2]:
		                                                                                            SelectedBox[3]]
