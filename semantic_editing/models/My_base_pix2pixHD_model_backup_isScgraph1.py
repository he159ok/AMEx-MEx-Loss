...
class MyBasePix2PixHDModel(BaseModel):
	...
	def forward(self, InComLableMap, InComInstMap, ObjPrior, InComImg, ComImage, feat):
		# Encode Inputs
		...
		pred_real = self.discriminate(input_label, ComImage)    #得设置成完整的对应完整的 但是 但是 但是 我自己设置的是不全的图片
		...
		return [self.loss_filter(loss_G_GAN.unsqueeze(0), loss_G_GAN_Feat.unsqueeze(0), loss_G_VGG.unsqueeze(0), loss_D_real.unsqueeze(0), loss_D_fake.unsqueeze(0)), fake_image]
	...
	def inference(self, InComLableMap, InComInstMap, ObjPrior, InComImg, ComImage, feat):
		# Encode Inputs
		input_label, inst_map, InComImg, ComImage, feat_map = self.encode_input(InComLableMap, InComInstMap, InComImg, ComImage, feat)
