...
class CLASSIFIER:
    # train_Y is interger 
    def __init__(self, _train_X, _train_Y, val_X, val_Y, input_nc, pool_nd, _nclass, _lr=0.001, _beta1=0.5, _nepoch=5, _batch_size=1):
        ...
        self.trindex_in_epoch = 0
        self.vlindex_in_epoch = 0
        self.epochs_completed = 0
        self.ntrain = len(self.train_X)
        self.nval = len(self.val_X)
        self.trperm = torch.randperm(self.ntrain)
        self.vlperm = torch.randperm(self.nval)
    ...
    def fit(self):
        for epoch in range(self.nepoch):
            for i in range(0, self.ntrain, self.batch_size):      
                ...
                batch_input, batch_label = self.next_batch_tr(self.batch_size, self.train_X, self.train_Y)
            ...
            acc_per_class, whole_acc = self.cal_acc(self.val_X, self.val_Y)
    ...
    def cal_acc(self, X, Y):
        ...
        for i in range(0, sample_num, self.batch_size):
            batch_input, batch_label = self.next_batch_vl(self.batch_size, X, Y)
            batch_input = batch_input.cuda()
    ...
    def next_batch_vl(self, batch_size, X, Y):
        start = self.vlindex_in_epoch
        sample_num = len(X)
        if start + batch_size > sample_num:
            # shuffle the data
            self.vlperm = torch.randperm(sample_num)
            # start next epoch
            self.vlindex_in_epoch = 0
            res_X = X[self.vlperm[self.vlindex_in_epoch]]
            res_Y = Y[self.vlperm[self.vlindex_in_epoch]]
            self.vlindex_in_epoch += batch_size
            return res_X, res_Y
        else:
            res_X = X[self.vlperm[self.vlindex_in_epoch]]
            res_Y = Y[self.vlperm[self.vlindex_in_epoch]]
    ...
    def next_batch_tr(self, batch_size, X, Y):
        start = self.trindex_in_epoch
        sample_num = len(X)
        if start + batch_size > sample_num:
            self.epochs_completed += 1
            # shuffle the data
            self.trperm = torch.randperm(sample_num)
            # start next epoch
            self.index_in_epoch = 0
            res_X = X[self.trperm[self.trindex_in_epoch]]
            res_Y = Y[self.trperm[self.trindex_in_epoch]]
            self.index_in_epoch += batch_size
            return res_X, res_Y
        else:
            res_X = X[self.trperm[self.trindex_in_epoch]]
            res_Y = Y[self.trperm[self.trindex_in_epoch]]
            self.trindex_in_epoch += batch_size
            # from index start to index end-1
            return res_X, res_Y
